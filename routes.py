...
@routes.route("/api/turnkey/email/request-code", methods=["POST"])
def turnkey_email_request_code():
    """Send email sign-in code via Supabase Auth OTP."""
    try:
        data = request.get_json() or {}
        ...
        supabase = get_supabase_client()
        if not supabase:
            return jsonify({"success": False, "error": "Email login unavailable right now"}), 503
        ...
        if referral_code:
            session["pending_email_referral"] = referral_code
        session.permanent = True

        return jsonify({"success": True, "message": "Code sent to your email."})
    except Exception as e:
        logger.error(f"Email OTP request error: {e}")
        return jsonify({"success": False, "error": "Failed to send email code"}), 500


@routes.route("/api/turnkey/email/verify-code", methods=["POST"])
def turnkey_email_verify_code():
    """Verify email OTP then create/login wallet session."""
    try:
        data = request.get_json() or {}
        email = (data.get("email") or "").strip().lower()
        code = (data.get("code") or "").strip()
        referral_code = (data.get("referral_code") or session.get("pending_email_referral") or "").strip().upper() or None

        if not email or not code:
            return jsonify({"success": False, "error": "Email and code are required"}), 400

        pending_email = (session.get("pending_email_login") or "").strip().lower()
        if pending_email and pending_email != email:
            return jsonify({"success": False, "error": "Email does not match requested login"}), 400
        ...
        if not getattr(verify_response, "user", None):
            return jsonify({"success": False, "error": "Invalid or expired code"}), 401

        # Stable ID for email-based user (no extra env secret needed).
        email_hash = hashlib.sha256(email.encode()).hexdigest()[:20]
        user_id = f"email_{email_hash}"

        from turnkey_service import create_turnkey_wallet
        result, err = create_turnkey_wallet(user_id, email)
        if err:
            ...
            session.pop("pending_email_referral", None)
            session.permanent = True
