...
FAUCET_FORCED_ONCHAIN_MAX_ATTEMPTS = int(os.getenv("FAUCET_FORCED_ONCHAIN_MAX_ATTEMPTS", "3"))
...
def faucet_gas():
    ...
    try:
        ...
        if force_onchain:
            logger.warning(
                f"⚠️ Faucet force_onchain enabled wallet={checksum_wallet.lower()} "
                f"correlation_id={correlation_id}"
            )
            attempts = 0
            onchain_result = None
            flow_result["attempted_onchain"] = True
            while attempts < max(1, FAUCET_FORCED_ONCHAIN_MAX_ATTEMPTS):
                attempts += 1
                onchain_result = _execute_onchain_faucet_topup(w3, checksum_wallet, correlation_id=correlation_id)
                if not onchain_result.get("success"):
                    break
                status_check = _get_gas_status(w3, checksum_wallet)
                if status_check.get("gas_ready"):
                    break
            status_after_forced = _get_gas_status(w3, checksum_wallet)
            post_balance_wei = int(status_after_forced["balance_wei"])
            flow_result["onchain_result"] = onchain_result
            flow_result["topup_source"] = "onchain" if (onchain_result or {}).get("success") else None
            terminal_status = (
                "gas_ready" if status_after_forced["gas_ready"] else
                ("not_configured" if (onchain_result or {}).get("reason") == "not_configured" else "onchain_failed")
            )
            return jsonify({
                "success": bool(status_after_forced["gas_ready"] or ((onchain_result or {}).get("success"))),
                "wallet": checksum_wallet.lower(),
                "gas_ready": status_after_forced["gas_ready"],
                "topped_up": bool((onchain_result or {}).get("success")),
                "topup_source": flow_result["topup_source"],
                "api_tx_hash": None,
                "api_error": None,
                "onchain_result": onchain_result,
                "status": "gas_ready" if status_after_forced["gas_ready"] else "onchain_sent",
                "attempted_api": False,
                "attempted_onchain": True,
                "api_result": None,
                "terminal_status": terminal_status,
                "correlation_id": correlation_id,
                "forced_onchain_attempts": attempts,
                "debug": {
                    "pre_balance_wei": str(pre_balance_wei),
                    "post_balance_wei": str(post_balance_wei),
                    "required_gas_wei": status_after_forced["required_gas_wei"],
                    "required_gas_celo": status_after_forced["required_gas_celo"],
                    "required_gas_reserve_wei": status_after_forced["required_gas_wei"],
                    "required_gas_reserve_celo": status_after_forced["required_gas_celo"],
                    "fallback_reason": "force_onchain",
                },
                **status_after_forced,
            })
